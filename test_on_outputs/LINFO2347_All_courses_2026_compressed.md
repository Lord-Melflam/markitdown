...
Identify: What are the critical assets in your system? (data,
...
Server verifies credentials and includes a session-token in
...
From now on, the browser includes the cookie value in every
...
In a different window, user visits a chat forum Y.com
2.
...
running out of TCP connections
...
Send an ICMP echo request (“ping”) packet to the target IP
address
If you get an ICMP echo reply packet back, you know that the
IP address is in use
If the host does not exist, an intermediate router might also
...
... wait for negative answer: If the UDP port is not open, the
...
Horizontal scan:
Find targets with open port
of interest

130.89.1.1

130.89.1.255 130.89.2.1
...
with 2 global 32-bit variables
...
is called, the following information (a stack
...
, the top stack frame is removed from
...
; push 4 onto the stack (4 bytes)
; push 3 onto the stack (4 bytes)
...
; save the framepointer on the stack
...
; SP = FP. This effectively removes
; the local variables from the stack

popl %ebp

; restores the old value of FP
...
▪ By providing a too long string

, you can overwrite the the saved
...
returns, the CPU will jump to the return address
...
2. Write the address of the system function into return
...
2. Observe flows with byte sizes b’1,b’2,b’3,…
3. What were the original byte sizes b1,b2,b3,… ?
...
• Missing one packet from a ten-packet flow

too bad, right?

10% error. Not

• Missing one packet from a one-packet flow
...
2.
If no entry found, insert a new entry to the table
...
If no entry found, drop the packet. Otherwise, replace
...
Split the mails into tokens (words) and calculate the token
2.
...
For a new mail, look at all its tokens and calculate the
probability that the mail is spam

Step 3

▪ Probability that an e-mail is spam if it contains token
...
▪ Paul Graham’s original mail filter assumed that
...
ς 𝑝𝑖/𝑃(𝑖𝑠 𝑆𝑝𝑎𝑚)
ς 𝑝𝑖/𝑃(𝑖𝑠 𝑆𝑝𝑎𝑚) + ς 1 − 𝑝𝑖 𝑃(𝑖𝑠 𝑛𝑜𝑡 𝑆𝑝𝑎𝑚)

:

𝑃 𝑖𝑠 𝑆𝑝𝑎𝑚 = 𝑃 𝑖𝑠 𝑛𝑜𝑡 𝑆𝑝𝑎𝑚
...
Correction factor 2 avoids too many false positives
 ...
 token freq. in good mails (or 0 if no occurrence)

𝑃 𝑖𝑠 𝑆𝑝𝑎𝑚 < 𝑃 𝑖𝑠 𝑛𝑜𝑡 𝑆𝑝𝑎𝑚 .

token freq. in bad mails (or 0 if no occurence)
...
architecture 
Intrude the botnet and send switch-off command
...
▪ Symmetric Cipher = efficient algorithms
,
...
: The key is preshared

𝑘

Perfect security

▪ Perfect security = the ciphertext
...
▪ It can be shown that if the symmetric cipher

𝐸(𝑘, 𝑚2)
secure, then key length
...
is a random bit string at least as long as the
...
•
• Human languages contain enough redundancy to recover

𝑐2

𝑐1 ⊕ 𝑐2 = 𝑚1 ⊕ 𝑘 ⊕ 𝑚2 ⊕ 𝑘 = 𝑚1 ⊕ 𝑚2
and
1. Make a guess what
𝑚1 ⊕ 𝑚2:
from
...
are generated using the output of a pseudo-
...
can happen
• if the users are not careful and use the same short key
...
▪ The attacker can try all possible

 combinations of the shorter

key and check which decrypted text looks right
• However: if the key length
...
,  key size = 168 bits
  key size = 128, 192, 256 bits (AES-128, AES-
...
transformation, the round function,
•
• The original key is expanded to provide round keys
...
▪ The original DES has
▪ A block of 64 bits is split into two 32-bit blocks

 rounds and a 56-bit key
...
▪ Challenge: Given a message and its ciphertext, find the key
...
▪ 2008: RIVYERA (successor of COPACOBANA).
...
𝑘1, 𝑘2, 𝑘3 , 𝑚 = 𝐸 𝑘1, 𝐷 𝑘2, 𝐸 𝑘3, 𝑚

= 3 x 56 = 168 bits

𝑘1, 𝑘2, 𝑘3

▪ The effective key length is only 112 bits if attacker knows

and

• Attacker computes all

 possible combinations

 and compares with all
...
56+56
→
 of a cipher does not always give better security

• Such a Meet-in-the-Middle attack is the reason why repeating

 encryptions/decryptions needed
...
128, stream cipher 2078

Hardware (SSE2)

Salsa20

256, stream cipher 1008

Hardware (SSE2)

ChaCha20

256, stream cipher 2185
...
• Each user would have to remember
 keys in the system
•
...
≔ 𝐸 𝑘𝑏, "A→B"+𝑘𝐴𝐵
5. Bob can decrypt the ticket and get the
6. Alice and Bob can now communicate using
𝑘𝐴𝐵
C
...
• Receiver decrypts using a secret (private) key
...
▪ A Public-Key Cryptosystem consists of
 ...
 randomized algorithm to generate
...
1. Choose randomly two large prime numbers
...
must be also kept secret (or deleted immediately after key
...
▪
mod 𝑛
▪ Security is based on the fact that it is hard to find
mod 𝑛
...
▪ Let’s see what happens if we decrypt
...
▪ Example: A computer uses RSA to encrypt a 64-bit message
𝐸 𝑝𝑘, 𝑚1 ⋅ 𝑚2 = 𝐸 𝑝𝑘, 𝑚1 ⋅ 𝐸 𝑝𝑘, 𝑚2

• With 20% probability,
𝑐 = 𝐸(𝑝𝑘, 𝑚)
both 34-bit numbers
• Attacker builds a table with
...
• Attacker goes through all

 and checks whether
 is in the table. When a matching entry is found, we
...
Append random padding  to message
...
Both sides calculate a shared secret
...
mod 𝑝 = 𝑔
mod 𝑝
as shared key for symmetric encryption
...
▪ Suprisingly, it is very hard for an eavesdropper to calculate
...
is not just some integer, it is a point on an
...
random (probably distinct) messages
...
messages with same hash with probablity
...
brute-force hash evaluations
evaluations was found
...
• Takes a message block

and result of previous

computation

and computes
𝑓
is collision-resistant, so is
...
1. Sender calculates tag

∀𝑘 ∈ 𝐾, 𝑚 ∈ 𝑀: 𝑉 𝑘, 𝑚, 𝑆 𝑘, 𝑚 = 𝑦𝑒𝑠
...
•
• Bad! If attackers finds a collision
...
Attacker can extend the message
𝑆 𝑘, 𝑚 = 𝐻(𝑘||𝑚)
the correct
...
▪ Goal: Don't rely on the collision-resistance of
▪ Basic idea of HMAC: Hide
...
• If
 ...
 is a pseudo-random function (PRF), then so is HMAC
...
(Optionally, C sends its own certificate to S)
...
▪ Diffie-Hellman is used to generate a Premaster Secret
...
7. Client sends
as PS
8. Client and server use
mod 𝑝
,  C and S compute a Master Secret with
9. Using PS,
𝑎𝑏
a Pseudo Random Function (PRF, e.g. SHA-256)
𝑔
10. From the Master Secret, symmetric session keys
...
for encryption (one for each direction) and MAC
𝐾𝐶

for MAC are created with another PRF
...
5. C generates 368-bit "Premaster Secret"
6. C sends
7. C and S compute symmetric session keys

𝑃𝑆

to S, RSA-encrypted with public key of S

 and

 (one

for each direction) using PS,
...
• Design errors, e.g., a TLS-protected "Pay" button on an
 ...
 Man-in-The-Middle can modify

unencrypted web page
the web page
...
predictable (bias)
 ...
 attacker can recover the plaintext
...
list of known crypto protocols and
...
the chosen crypto protocol and
its own part of the data for the handshake
the X.509 certificate encrypted with
4. C verifies certificate of S and computes
...
▪ Vulnerable to replay attacks! No random numbers
...
TC: truncated because of MTU limit
RD: Recursion desired
RA: Recursion available

(From DNSSEC:)
AD: Authentic data (resolver believes
the response was authenticated)
CD: Checking Disabled (resolver did
not check signature)
...
• Answer contains no or wrong RRSIG
...
and DNSSEC when using ANY queries
support anymore ANY

 Some resolvers do not
...
and payload are Base64url
encoded (RFC 4648). It’s like
Base64, but with special
...
• To stop an application to use your data, you would have to
 ...
 requires to change credentials (=the
...
execution, and file transfer; also Zebrocy, CHOPSTICK,
GAMEFISH

Persistence

Backdoors, rootkits, OAuth token theft

C2

Encrypted channels blending into normal HTTPS traffic
...
(email, authentication tokens, directory services) enabling
